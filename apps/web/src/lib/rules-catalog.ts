...
export type RuleCategory =
  ...
  | 'projects'       // long-running expense buckets, exclusion from monthly
...
export const RULE_CATEGORIES: Record<RuleCategory, { label: string; emoji: string; description: string }> = {
  ...
  projects: {
    label: 'פרויקטים (דליי הוצאה לטווח ארוך)',
    emoji: '📦',
    description: 'איך פרויקטים גדולים (בנייה, חתונה) חיים בנפרד מהתזרים החודשי, כדי שלא יטביעו את ההוצאות הרגילות.',
  },
  ui: {
    label: 'תצוגה וחוויה',
    emoji: '🎨',
...
export const RULES_CATALOG: RuleEntry[] = [
  ...
  {
    id: 'installment-bank-double-listed',
    title: 'בנקים מציגים תשלום אחד כמה פעמים — האפליקציה משמיטה כפילויות',
    category: 'installments',
    story:
      'חלק מקבצי האשראי (בעיקר דיינרס/אקספרס) מציגים את כל תשלומי תוכנית ' +
      'התשלומים כשורות נפרדות באותו תאריך — למשל קנייה של ₪8,000 ב-4 ' +
      'תשלומים תופיע כ-4 שורות זהות של ₪2,000 ב-7/1, כולן תחת "תשלום 1/4". ' +
      'אם כל אלו ייקלטו כעסקאות נפרדות, יראה לך 4 חיובים של ₪2,000 ב-7/1 ' +
      'במקום אחד. ' +
      'מאז גרסה 2026-05-07: כשה-importer מזהה שורות כפולות באותו ' +
      'תאריך/סכום/בית עסק/תוכנית תשלומים, הוא שומר רק אחת. ' +
      'התשלומים החסרים (2/4, 3/4, 4/4) נוצרים אוטומטית כשורות "צפוי" ' +
      'בחודשים הבאים על-ידי לוגיקת הסינתזה.',
    why: 'מדובר בשגיאת פורמט של חלק מקבצי הבנקים. בעבר הייתה תוצאה של 4 ' +
         'שורות זהות שמטעות את התקציב; עכשיו זה מוסטר.',
    example: '"ויהי אור אדיסון" — קובץ דיינרס הציג ₪2,000 × 4 שורות באותו ' +
             '7/1 כל אחת מסומנת "תשלום 1/4". האפליקציה שומרת אחת, השאר ' +
             'מוצגות אוטומטית כ"צפוי" בפב/מרץ/אפר.',
    code: ['apps/web/src/app/(app)/import/actions.ts', 'packages/db/src/dedupe-installment-duplicates.ts'],
  },

  // ── PROJECTS ───────────────────────────────────────────────────────
  {
    id: 'projects-exclude-from-monthly',
    title: 'תנועות פרויקט אינן נספרות בתצוגות החודשיות',
    category: 'projects',
    story:
      'כשאתה מתייג תנועה לפרויקט (למשל "בניית בית"), היא נעלמת ' +
      'מלוח המחוונים, מדף התנועות, ומתצוגות אישי / עסקי / משולב — ' +
      'ומופיעה רק בדף הפרויקט שלה ב-/projects/[id]. ברירת המחדל של ' +
      'כל פרויקט חדש: excludeFromMonthlyTotals = true.',
    why: 'העברת ₪200,000 לקבלן ביום אחד היא אירוע חריג, לא תזרים שוטף. ' +
         'אם נספור אותה בהוצאות החודש, "תקציב החודש" יראה -₪180,000 ' +
         'ויטביע את ההוצאות הרגילות שעליהן באמת אפשר לפעול.',
    example: '"הע. לס ב חולדה פרו" -₪212,400 שתיוגת לפרויקט "בניית בית": ' +
             'לא תופיע בלוח המחוונים החודשי, לא תוכלל בתחזית סוף חודש, ' +
             'לא תופיע בהשוואה חודש-מול-חודש. תופיע רק ב/projects/[id].',
    code: [
      'apps/web/src/lib/project-filter.ts',
      'apps/web/src/app/(app)/page.tsx',
      'apps/web/src/app/(app)/transactions/page.tsx',
    ],
  },
  {
    id: 'projects-include-in-monthly-override',
    title: 'תיוג כפול: תנועה גם בפרויקט וגם בסיכומים החודשיים',
    category: 'projects',
    story:
      'בעריכת תנועה שמתויגת לפרויקט יש checkbox "📦 כלול גם בסיכומים החודשיים". ' +
      'כשהוא מסומן, התנועה תופיע גם בלוח המחוונים ובדף התנועות (כסף שיצא ' +
      'החודש), ולא רק בדף הפרויקט. כך אפשר לחלק תנועות פרויקט לשני סוגים:\n' +
      '• Capex (הוצאה הונית): העברה של ₪200,000 לקבלן — רק בפרויקט\n' +
      '• Opex (הוצאה שוטפת): מנורה של ₪400 לבית בבנייה — בשני המקומות',
    why: 'פרויקטים גדולים אינם דבר אחד אחיד. חלק מהתנועות הן באמת מחוץ ' +
         'לתזרים החודשי הרגיל (העברות גדולות לקבלן), אבל חלק מהן הן הוצאות ' +
         'שוטפות שכן צריך לראות בתקציב החודש (קניית פריט קטן בחנות חומרי ' +
         'בניין). הדגל הזה נותן שליטה מדויקת.',
    example: 'פרויקט "בניית בית" עם excludeFromMonthlyTotals=true. ' +
             'תנועה ₪212K לקבלן: includeInMonthlyOverride=false (ברירת מחדל) → ' +
             'מוצגת רק ב/projects/[id]. תנועה ₪400 על מנורה לבית: ' +
             'includeInMonthlyOverride=true → מוצגת גם בלוח המחוונים, גם ' +
             'ב/transactions, וגם ב/projects/[id]. בדף הפרויקט יש תג ' +
             '"📅 גם חודשי" ליד שם בית העסק כדי לזהות תנועות כאלה.',
    code: [
      'packages/db/src/helpers/project-filter.ts',
      'apps/web/src/app/(app)/transactions/edit-modal.tsx',
    ],
  },
  {
    id: 'projects-tagging-ui',
    title: 'תיוג תנועות לפרויקט — שורה בודדת או בכמות',
    category: 'projects',
    story:
      'בכל שורת תנועה ב-/transactions יש כפתור 💼 (תיק) שפותח רשימת ' +
      'הפרויקטים הפעילים שלך. לחיצה תייגת את התנועה לפרויקט; אם השורה ' +
      'כבר מתויגת, יש גם אפשרות להסיר תיוג. ' +
      'בבחירה של כמה שורות (תיבות הסימון), אפשר לתייג את כולן בבת ' +
      'אחת באמצעות "תייג לפרויקט" בסרגל הפעולות.',
    why: 'תיוג תנועות הוא הפעולה הכי חשובה בפרויקט — חייב להיות נגיש ' +
         'בלי לעבור למסך אחר.',
    code: [
      'apps/web/src/app/(app)/transactions/assign-to-project-menu.tsx',
      'apps/web/src/app/(app)/projects/actions.ts',
    ],
  },
  {
    id: 'projects-delete-unassigns',
    title: 'מחיקת פרויקט מחזירה תנועות לתזרים הרגיל',
    category: 'projects',
    story:
      'מחיקת פרויקט (כפתור 🗑 ב-/projects) קודם משייכת את projectId ל-NULL ' +
      'בכל התנועות שהיו תחתיו, ורק אז מוחקת את שורת הפרויקט. ' +
      'התנועות עצמן נשארות במסד הנתונים — הן פשוט חוזרות להופיע ' +
      'בתצוגות החודשיות הרגילות.',
    why: 'כדי שלא תאבד תנועות בטעות. אם החלטת שהפרויקט לא רלוונטי יותר ' +
         '(או חולק לכמה פרויקטים קטנים), אתה רוצה שהתנועות יישארו.',
    code: ['apps/web/src/app/(app)/projects/actions.ts'],
  },
  {
    id: 'projects-budget-tracking',
    title: 'תקציב פרויקט אופציונלי — מעקב אחוזים והתראות',
    category: 'projects',
    story:
      'אפשר להגדיר תקציב כולל לפרויקט (totalBudgetIls). אם הוגדר, ' +
      'דף הפרויקט מציג סרגל התקדמות — ירוק עד 80%, צהוב 80%-99%, ' +
      'אדום ≥100%. בלי תקציב מוגדר, מציגים רק את סך ההוצאה בפועל.',
    why: 'פרויקטים גדולים בדרך כלל מתחילים עם הערכה כללית של עלות. ' +
         'מעקב אחוזים עוזר לזהות חריגה לפני שהיא יוצאת מכלל שליטה.',
    code: ['apps/web/src/app/(app)/projects/[id]/page.tsx'],
  },
  ...
  {
    id: 'income-expense-sign-based',
    title: 'הכנסות והוצאות מחושבות לפי סימן הסכום, לא לפי סוג הקטגוריה',
    category: 'transactions',
    story:
      'בכרטיסי "הכנסות" ו-"הוצאות" בלוח המחוונים, הסיווג נעשה לפי סימן ' +
      'הסכום בלבד:\n' +
      '• סכום חיובי → "הכנסה" (כסף שנכנס)\n' +
      '• סכום שלילי → "הוצאה" (כסף שיצא)\n' +
      'הקטגוריה לא משפיעה על הסיווג. כך מבטיחים שהמאזן (הכנסות − הוצאות) ' +
      'תמיד שווה לסך התנועות בפועל, גם אם תייגת תנועה בקטגוריה הלא נכונה.',
    why: 'בעבר השתמשנו בסיווג לפי category.isIncome. הבעיה: כשתנועה חיובית ' +
         'גדולה (למשל העברה בין חשבונות שלא סומנה כ-isTransfer) מתויגת ' +
         'בקטגוריית הוצאה כמו "הלוואות וחיסכון", היא הייתה מקטינה את ' +
         'ההוצאות במקום להגדיל את ההכנסות. תוצאה: ההוצאות נראו נמוכות ' +
         'מדי וההכנסות נמוכות מדי, אבל המאזן עדיין יצא נכון מקרית.',
    example: 'תנועה: ₪+45,489 מבנק הפועלים ללאומי עו״ש עסקי, מתויגת ' +
             '"הלוואות וחיסכון". זה למעשה העברה בין חשבונות שלך. ' +
             'בלוגיקה החדשה: נספרת בהכנסות (סימן חיובי), ובאנר אזהרה ' +
             'מתריע שמומלץ לסמן אותה כ"העברה" כדי להוציא אותה משני ' +
             'הצדדים בטאב משולב.',
    code: ['apps/web/src/app/(app)/page.tsx'],
  },
  {
