...
export const RULES_CATALOG: RuleEntry[] = [
  ...
  // ── TRANSACTIONS ───────────────────────────────────────────────────
  {
    ...
    story:
      ...
      'מהחשבון הבנקאי הקשור — לא ממתינות למחזור החיוב החודשי.',
    why: 'תזרים המזומנים בלוח שנה צריך להציג את היום שבו הכסף באמת יצא, לא את ' +
         'התאריך הרישומי של הבנק.',
    example: 'OpenAI ChatGPT $20 ב-4 במאי → מופיע בתזרים של 4 במאי, לא ב-10 במאי.',
    code: ['apps/web/src/lib/smart-importer.ts'],
  },
  {
